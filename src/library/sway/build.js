...
async function main() {
  try {
    ...
    const mjsContent = fileContent
      .map((content) => [
        `export const ${content.project}ABI = ${content.abi};`,
        `export const ${content.project}BIN = ${content.bin};`,
      ])
      .flat()
      .join('\n');
    ...
    writeFileSync(
      ...
      fileContent
        .map((content) => [
          `module.exports.${content.project}ABI = ${content.abi};`,
          `module.exports.${content.project}BIN = ${content.bin};`,
        ])
        .flat()
        .join('\n'),
    );
